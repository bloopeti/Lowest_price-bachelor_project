...
def load_product_url(product_url, domain):
    url = "http://localhost:9906/app/productUrl/find"
    response_for_url = requests.post(url=url, data=product_url)
    product_id = json.loads(response_for_url.text)['productId']
